...
contract Ed25519Oracle is Ownable, ReentrancyGuard, IEd25519Oracle {
    ...
    constructor(uint256 _minimumStake, uint256 _consensusThreshold, uint256 _consensusTimeout, uint256 _maxDataAge)
        Ownable(msg.sender)
    {
     ...
     */
    function submitProof(bytes32 publicKey, bytes32 messageHash, bytes calldata signature, bool isValid)
        external
        onlyActiveNode
        nonReentrant
    {
     ...
     */
    function isVerified(bytes32 publicKey, bytes32 messageHash, bytes calldata signature)
        external
        view
        override(IEd25519Oracle)
        returns (bool)
    {
        bytes32 dataId = _hashDataId(publicKey, messageHash, signature);
    ...
    {
        NodeInfo storage node = nodes[nodeAddress];
        return (node.isActive, node.stake, node.reputation, node.lastActivity, node.isRegistered);
    }
    ...
    {
        return (nodeList.length, totalStake, minimumStake, consensusThreshold, consensusTimeout, maxDataAge);
    }
     ...
     */
    function _hashDataId(bytes32 publicKey, bytes32 messageHash, bytes calldata signature)
        internal
        pure
